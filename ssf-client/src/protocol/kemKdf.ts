import { appendBuffers, deriveAesGcmKey, deriveHKDFKeyWithDH, generateEphemeralKeyPair, generateSalt, subtle } from "./commonCrypto";
 ...
 */
export async function kemKdfEncap(pk: CryptoKey, keyLabel: ArrayBufferLike): Promise<KemKdfEncapResult> {
    if (pk.type !== 'public') {
 ...
 */
export async function kemKdfDecap({ privateKey: sk, publicKey: pk }: CryptoKeyPair, c: CtxtKem, keyLabel: ArrayBufferLike): Promise<CryptoKey> {
    // (pe, salt) <- C_kem
    const { pe, salt } = c;
...
async function calculateKemKdfLabel(pk: CryptoKey, pe: CryptoKey, keyLabel: ArrayBufferLike) {
