...
The goal of SSF is to enforce access control to file content with cryptography.
...
The motivation behind the SSF construction is:
\begin{itemize}
    \item E2EE is already regarded as a standard security guarantee for data in transit, providing integrity and confidentiality of such data. 
    For data in transit, endpoint compromise is not normally in scope, but in practice most applications decrypt and then store the data locally. A compromise at the endpoint of such E2EE systems would therefore result in a huge impact.
    \item The lifetime of persistent data is decoupled from a member lifetime, i.e., legitimate access to the group data.
    This is a major difference with respect to data in transit.
\end{itemize}

The investigation of the points above brings to the following
observations:
\begin{itemize}
    \item Forward secrecy (FS) is not suited for 
    persistent data: while users have access to some data, 
    they need to keep the key material to actually access it. 
    Generally, FS requires instead to securely delete such key
    material and the plaintext as soon as the data is consumed. 
    \item Post compromise security (PCS) is expensive for persistent data, as it
...
all removed users retain the capability of decrypting the
compromised data.
