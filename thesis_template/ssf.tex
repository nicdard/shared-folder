...
In this chapter we describe in details the Secure Shared Folder (SSF) scheme (\cref{sc:SSF-scheme}) and its implementation.
...
some group members are admins, and provide interval access control security (\cref{sc:in}) 
...
The implementation details are given in the reminder of the chapter.
...
We therefore re-implement the tree version of SSKG, whose pseudocode is present in~\cite{ESORICS:MarPoe14}, using TypeScript.
...
This also explain why the initial seed is generated as an HMAC SHA-256 key.
...
When invoking any API call where an HKDF or HMAC algorithm are involved,
...
always have fixed length of 256 bits, so we can use HMAC as a
double-PRF, to 
combine backward and forward chain elements together 
...
(that is equivalent to compute HMAC) $k_2$.
...
to assure that the bits the keys are fully utilised
...
the DKR syntax. In the same file we also provide the types for the various
...
As mentioned above, numbers are 64-bits double precision floating point in JS,
...
Notice that the chains are stored ordered by epoch of instantiation,
...
For practicioners, we highlight that
...
We implement the abstract functionalities of the DS inside our server existing SSF Gateway
...
\begin{itemize}
    \item \texttt{uid}: the \texttt{Uint8Array} representing the unique identifier of the user, this corresponds to the \texttt{user\_email} in the public certificate from the PKI and with which the user register itself to the SSF Gateway.
...
\begin{enumerate}
    \item Depending on the operation, the client propose and commit the changes internally to the CGKA group state, some of these operation involve both member and admin CGKA groups. The pending commit(s) is (are) written to the in-memory storage (inside Wasm).
    ...
    \item The DS process the \texttt{POST} proposal request (or v2 \texttt{PATCH} folders request) and answers according to whether the pending message queue for the sender in the targeted folder is empty or not:
    \begin{itemize}
        \item accepts the message and send back all the \texttt{messageId} created by the DB to the client;
        \item rejects the message and send back an error to the client.
    \end{itemize}
...
call~\cite{WebCryptoAPIImportKey}. In Chrome, the library BoringSSL~\cite{BoringSSL} underlying the Web Crypto API 
